...
# for the cgo build. macOS/Linux: fetch.sh.
#
...
# ---- normalize into deps\current (what #cgo points at) -------------------
# A stable directory name keeps the cgo flags platform-independent. The
# import library is copied under the libcorvid.dll.a name mingw-w64's ld
# searches first, so `-lcorvid` resolves without depending on ld's .lib
# suffix rules; corvid.dll must additionally be on PATH (or beside the
# binary) at runtime for the loader.

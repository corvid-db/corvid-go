...
// §4.4 addition — ascending key-BYTE order, whatever wrote the data):
// a decoded map is always COMPLETE. This collapsed the v0.2.2-era
// candidate-key oracle (a per-Db remembered key set probed by known
// key, loud ErrMapKeyEnumeration failure on unknown keys — see
// docs/PLAN.md's decision-log row); Get, Scan, Update callbacks, and
// query rows now decode anything the engine can read, on any database.
...
// Go-owned value. Maps enumerate their keys through
// corvid_value_map_keys — every entry decodes, unknown keys included.
func decodeValue(h *cValueHandle) (any, error) {
	switch cVType(h) {
	case tagNull:
		return nil, nil
	case tagBool:
	...
	case tagArray:
		...
		out := make([]any, 0, n)
		for i := 0; i < n; i++ {
			item, err := decodeValue(cVArrayGet(h, i))
			if err != nil {
				return nil, err
			}
			out = append(out, item)
	...
	case tagMap:
		...
		keys, err := cVMapKeys(h)
		if err != nil {
			return nil, err
		}
		if len(keys) != n {
			return nil, newErr(ErrDecode, "map decode: %d of %d keys enumerated", len(keys), n)
		}
		out := make(map[string]any, n)
		for _, k := range keys {
			child := cVMapGet(h, k)
			if child == nil {
				return nil, fmt.Errorf("corvid: map decode: enumerated key %q absent", k)
			}
			item, err := decodeValue(child)
			if err != nil {
				return nil, err
			}
			out[k] = item

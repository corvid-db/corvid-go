#!/usr/bin/env bash
# fetch.sh — download, VERIFY, and extract the pinned corvid FFI release for
# this host (macOS/Linux), then normalize the artifacts into deps/current
# for the cgo build. Windows: fetch.ps1.
#
...
# Required: corvid.h always; plus the cdylib for this platform.
...
# A stable directory name keeps the cgo flags platform-independent.
